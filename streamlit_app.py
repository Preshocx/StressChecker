...
def main():
    st.set_page_config(page_title="Stress Tracker", page_icon=":chart_with_upwards_trend:")
...
def show_stress_log():
    ...
    # Display the stress log chart
    if not stress_log_data.empty:
        st.subheader("Stress Log Entries")
        fig = px.line(stress_log_data, x='Date', y='Stress Level', title='Stress Level Over Time')
        st.plotly_chart(fig)

        st.subheader("Stress Log Table")
